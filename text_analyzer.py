...
def total_letters(text):
    ...
    """
    cantidadLetras = count_vowels(text) + count_consonants(text)
...
def vowel_percentage(text):
    ...
    if total_letters(text) == 0:
        ...
        porcentajeVocales = round((count_vowels(text) * 100) / total_letters(text), 1)
...
def analyze_text(text):
    ...
    """
    textoCompleto = f"V:{count_vowels(text)} C:{count_consonants(text)} T:{total_letters(text)} P:{vowel_percentage(text)}%"
